...
const DEFAULT_FROM_EMAIL = 'ReviewGuard <noreply@getreviewguard.com>';
...
async function getCredentialsFromReplit() {
  ...
  try {
    ...
    console.log('Replit connector not available, falling back to direct API key');
    return null;
  }
}
...
async function getCredentials() {
  ...
  throw new Error('No Resend credentials available. Set RESEND_API_KEY environment variable.');
}
...
export async function sendWelcomeEmail(email, plan) {
  try {
    ...
    await client.emails.send({
      from: fromEmail,
      to: email,
      subject: 'Welcome to ReviewGuard! 🎉',
      html: `
        <h2>Welcome to ReviewGuard!</h2>
        ...
        <p>Best regards,<br>The ReviewGuard Team</p>
      `
    });
  ...
  try {
    ...
    await client.emails.send({
      ...
      html: `
        ...
        <p>We were unable to process your recent payment for your ReviewGuard subscription.</p>
...
export async function sendPasswordResetEmail(email, resetToken, companyName, baseUrl) {
  try {
    ...
    await client.emails.send({
      from: fromEmail,
      to: email,
      subject: '🔐 Password Reset Request - ReviewGuard',
      html: `
        <h2>Password Reset Request</h2>
        <p>Hello from <strong>${companyName || 'ReviewGuard'}</strong>,</p>
