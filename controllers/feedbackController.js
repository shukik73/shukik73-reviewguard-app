...
export const submitInternalFeedback = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[INTERNAL FEEDBACK] Received Token: ${feedbackToken}, Rating: ${rating}, Feedback: ${feedbackText}`,
    );
    ...
    if (ratingNum < 1 || ratingNum > 3) {
      return res.status(400).json({
        success: false,
        error:
          "Internal feedback is only for ratings 1-3. Use public review endpoint for higher ratings.",
      });
    }
    ...
    if (customerResult.rows.length > 0) {
      ...
      console.log(
        `[INTERNAL FEEDBACK] Found customer by tracking_token: ${customer.name}`,
      );
      ...
      console.log(
        `[INTERNAL FEEDBACK] ✅ Saved feedback for customer ${customer.name}`,
      );
      return res.json({
        success: true,
    ...
    console.log(
      `[INTERNAL FEEDBACK] Token lookup result: ${messageResult.rows.length} rows found`,
    );
    ...
    if (!message.user_id) {
      console.error(
        `[INTERNAL FEEDBACK] ❌ CRITICAL: Message ${message.id} has NULL user_id. Cannot save feedback.`,
      );
      console.error(
        `[INTERNAL FEEDBACK] This indicates the original message was created without user_id - data integrity issue!`,
      );
      return res.status(500).json({
        success: false,
        error:
          "Unable to process feedback due to data integrity issue. Please contact support.",
      });
    }

    console.log(
      `[INTERNAL FEEDBACK] ✓ Linking Feedback to User ID: ${message.user_id}`,
    );
    ...
    if (message.user_email) {
      ...
      await sendEmail(message.user_email, emailSubject, emailBody).catch(
        (err) => console.error("Failed to send low rating alert email:", err),
      );
    }
...
export const submitPublicReview = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[DEBUG] submitPublicReview - Token received: ${feedbackToken}`,
    );
    ...
    if (ratingNum < 4 || ratingNum > 5) {
      return res.status(400).json({
        success: false,
        error:
          "Public reviews are only for ratings 4-5. Use internal feedback endpoint for lower ratings.",
      });
    }
    ...
    if (customerResult.rows.length > 0) {
      ...
      console.log(
        `[PUBLIC REVIEW] Customer ${customerResult.rows[0].name} rated ${ratingNum} stars`,
      );
      return res.json({
        success: true,
    ...
    if (result.rows.length === 0) {
      console.log(
        `[DEBUG] Token not found in customers or messages table: ${feedbackToken}`,
      );
      return res.status(404).json({
        success: false,
        error: "Invalid feedback token",
...
export const getFeedback = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[GET FEEDBACK] ✓ Authenticated - Fetching feedback for user_id: ${userId}`,
    );
    ...
    const totalCount = await pool.query(
      "SELECT COUNT(*) as count FROM internal_feedback",
    );
    const orphanCount = await pool.query(
      "SELECT COUNT(*) as count FROM internal_feedback WHERE user_id IS NULL",
    );
    ...
    console.log(
      `  - Orphaned feedback (NULL user_id): ${orphanCount.rows[0].count}`,
    );

    if (result.rows.length === 0 && totalCount.rows[0].count > 0) {
      console.log(
        `[GET FEEDBACK] ⚠️  User has NO feedback but ${totalCount.rows[0].count} records exist in DB`,
      );
      if (parseInt(orphanCount.rows[0].count) > 0) {
        console.log(
          `[GET FEEDBACK] 🔍 FOUND ORPHANED DATA: ${orphanCount.rows[0].count} feedback records have NULL user_id`,
        );
      }
    }
...
export const trackLinkClick = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[TRACK LINK CLICK] ✅ Marked message ${message.id} as clicked for ${message.customer_name}`,
    );
...
export const markFeedbackAsRead = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[MARK FEEDBACK READ] User ${userId} marking feedback ${feedbackId} as read`,
    );
    ...
    console.log(
      `[MARK FEEDBACK READ] Successfully marked feedback ${feedbackId} as read`,
    );
...
export const blockFeedback = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[BLOCK FEEDBACK] User ${userId} blocking feedback ${feedbackId}`,
    );
    ...
    console.log(
      `[BLOCK FEEDBACK] Successfully marked feedback ${feedbackId} as ignored`,
    );
...
export const markSmsSent = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[MARK SMS SENT] User ${userId} marking feedback ${id} as SMS sent`,
    );
    ...
    console.log(
      `[MARK SMS SENT] Successfully marked feedback ${id} as SMS sent at ${result.rows[0].sms_sent_at}`,
    );
...
export const markCalled = (pool) => async (req, res) => {
  try {
    ...
    console.log(
      `[MARK CALLED] User ${userId} marking feedback ${id} as called`,
    );
    ...
    console.log(
      `[MARK CALLED] Successfully marked feedback ${id} as called at ${result.rows[0].called_at}`,
    );
...
export const assignFeedback = (pool) => async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      assignedTo,
      status: result.rows[0].feedback_status,
    });
  } catch (error) {
...
export const getGroupedFeedback = (pool) => async (req, res) => {
  try {
    ...
    for (const phone in grouped) {
      ...
      customer.avg_rating = (
        customer.feedback.reduce((sum, f) => sum + f.rating, 0) /
